...
type Scope = "year" | "all";
interface TopRow { rank: number; name: string; initials: string; role: string; points: number; isMe: boolean }
        ...
        <div className="divide-y divide-white/[0.06]">
          ...
          {data?.top.map((row, i) => (
            <div
              ...
              className={`grid grid-cols-[64px_1fr_auto] items-center gap-4 px-5 py-3 sm:grid-cols-[64px_1.4fr_1fr_auto] sm:px-6 ${row.isMe ? "bg-emerald-500/[0.07]" : ""}`}
            >
              {/* Rank */}
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-[14px] font-semibold">{row.name}{row.isMe && <span className="ml-2 rounded-full bg-emerald-500/20 px-1.5 py-0.5 text-[10px] font-bold text-emerald-300">Kamu</span>}</p>
